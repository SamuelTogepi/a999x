# A999X Activator: A9, iOS 9, 9 Years Later

_By Samuel Bowers_

A completely automatic solution that tether downgrades to iOS 9.2-9.3.3, jailbreaks, and activates any iPad Pro 12.9" and 9.7" Made possible by the work of many [others](#credits). iOS 9 WILL NEVER DIE. My Mom had an iPad Pro 9.7", hence why I made this.

For those unfamilar, A9X iOS 9 activation doesn't work normally anymore for many iPads and no one really knows why. Apple seems to have broken something during downgrade party. After [turdus_merula](https://sep.lol) dropped there were more iOS 9 users then there had been in years, many now expierencing the activation issue (including myself). A999Activator takes all the public knowlege on activating A9X iOS 9 devices and uses some [new](#how-this-works) techniques to make the proccess as seemless as possible for the end user. I hope you enjoy this as much as I. I've been out of the scene mostly for years, so if you can contribute to this and make it better please do!
...
If you have an iPad Pro 12.9" or 9.7", is it is recommended to downgrade to iOS 9.2 or iOS 9.2.1. These versions have everything working:
...
* YouTube in Safari works. The Safari browser in iOS 9 is so old that ads are broken and never show, but videos do play if you put the iPad in a landscape orientation.

* You can use iOS 9.2 and iOS 9.2.1 to send/recieve FaceTime calls and iMessages to your main iPad.

* You can use it as your main iPad (I did for a few weeks). T-Mobile/Mint Mobile in the USA are confirmed working carriers.

## FAQ

### My iPad Won't Detect On Fedora Linux
...
### My iPad Is Stuck In The Ramdisk Ran Right After iOS 9 Is Restored.
...
### How Do I Boot My Downgraded iOS 9 iPad If The Battery Dies Or I Turn It Off?

In the same folder that the `a999` command is in, there will be a new command starting with `boot` which is generated after a successful downgrade automatically. All you have to do is execute that command starting with `boot` while your iPad is in Recovery Mode and connected to your computer.

### Why Is My iPad Not Detected?
...
On the first run of a999xactivator, there are many additional steps in the proccess that will trigger automatically for you. **Subsequent runs will be signifigantly shorter and take fewer steps as it caches needed data from the first run locally in the `data` folder.** That `data` folder is very important and personalized to your iPad. You should back it up because you can put it back in any future a999xactivator release and it will use that data when it detects your iPad!

Similar to the data folder are the `boot*` scripts. These get generated once your iPad downgrades successfully automatically, and are also personalized to your iPad and should be backed up so you can reboot the iPad from Recovery Mode if it dies or your turn it off.

You can also use the `a999x` command to transfer your `data` and `boot*` files from a previous release of A999X Activator:

`./a999x.sh -u <path to new update of a999x activator>`

### How Are Errors Handled?

Certian aspects of Turdus_ra1n (exploiting SEP, booting exploited iOS) can fail the first time. This is why A999Xactivator has very extensive if-fail-then-retry logic. It will eventually work, and it won't continue the proccess until it does. So don't be discouraged when it says `Something went wrong, lets try that again` because it's really just working as intended and trying again (sometimes many times to get that PTEBlock) does eventually work out. One exception to this is if turdusra1n/turdus_merula crashes at `- <Log> checkm8 setup stage`. If your stuck here for a long time (more then 30 seconds) I would `ctrl+c` to exit a999Xactivator, unpulg the USB-A to Lightning cable from the USB port on the Mac, then plug it back in before running the `a999X` command again. Unfortunately I don't have a better solution for this yet as it is a turdus merula problem. In a similar vein to above, if you don't enter DFU mode when prompted or the custom ramdisk doesn't boot, a999Xactivator notices this and goes back to correct it.
...
So iOS 9.0.x/iOS 9.1/9.3.4/9.3.5 have jailbreaks. The problem here is that these jailbreaks require an activated iOS 9 iPad. Chicken and egg problem, we need a jailbreak to activate. This could be developed in the future if it can be done from a ramdisk entrypoint similar to how iOS 9.2-9.3.3 are handled and then triggered with some kind of untether or Safari exploit.

## How This Works

Remember, this is all automatic (as possible)!

1) Restores the latest iOS if your iPad is not on the latest iOS and activation files have not yet been backed up.
...
6) Boots a custom ramdisk in Recovery mode that puts the activation tarball files, an activation script, and a launch daemon all on /. Jailbreaks the iPad (needs a trigger to enable it but bootstrap is installed), disables Setup.app, and then reboots into Recovery Mode.

7) Boots iOS 9.2-9.3.3.

8) User is prompted to sign in to Wi-Fi and then go to http://jbme.ddw.nu to enable the Jailbreak.

9) Jailbreak triggers the previously in-active launch daemon. Launch daemon extracts all activation tarball files that were put on / into the proper /var places. It then modifies a plist file for activation needed for iOS 9.3.x. After that it deletes itself and all other temp files. Setup.app is then re-enabled, and the iPad is rebooted into Recovery mode.

10) iPad is booted into iOS 9.2-9.3.3. Activation status is checked and if successful a special boot script is created dynamically in the same directory as the a999X command which can be used to boot the device from Recovery Mode in the future.
...
The A999X activator itself is released under the 3-BSD license; see [license.md](license.md). A999XActivator uses many other dependency programs which are not under that license, such as:
